...
public class WindowApprovalLoan extends WindowMain {
	private static WindowApprovalLoan instance = new WindowApprovalLoan();

	public static WindowApprovalLoan getInstance() {
		return instance;
	}

	public WindowApprovalLoan() {
		window = new Window();
		window.setId("WindowApprovalLoan");
	...
	public void addComponents() {
		...
		proxy = new RpcProxy<PagingLoadResult<ViewAssignedLoanBeanModel>>() {
			...
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewAssignedLoanBeanModel>> callback) {
				...
				interfaceAsync.getApprovalLoanPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewAssignedLoanBeanModel>>() {
					@Override
		...
		columns.add(new ColumnConfig("tbeEmployeeId", "<center><b>Employee ID</b></center>", 150));
				...
				if (data == 0) {
					...
					Button btnView = new Button("", new SelectionListener<ButtonEvent>() {
						...
						public void componentSelected(ButtonEvent ce) {
							...
							WindowApprovalLoanApprovalView window = WindowApprovalLoanApprovalView.getInstance();
							window.show(id);
						}
					});
					btnView.setToolTip("approval view");
					...
					Button btnView = new Button("", new SelectionListener<ButtonEvent>() {
						...
						public void componentSelected(ButtonEvent ce) {
							int id = Integer.parseInt(model.get("tbaloId").toString());
							WindowApprovalLoanView window = WindowApprovalLoanView.getInstance();
							window.show(id);
						}
					});
