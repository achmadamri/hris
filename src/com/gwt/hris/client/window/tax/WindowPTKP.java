...
public class WindowPTKP extends WindowMain {
	private static WindowPTKP instance = new WindowPTKP();

	public static WindowPTKP getInstance() {
		return instance;
	}

	public WindowPTKP() {
		window = new ContentPanel();
		window.setId("WindowPTKP");
	...
	RpcProxy<PagingLoadResult<TbPtkpBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbPtkpBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbPtkpBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deletePtkp(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbPtkpBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbPtkpBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbPtkpBeanModel data = (TbPtkpBeanModel) itr.next();
					ids[i] = data.getTbptkpId();
					i++;
				}
				interfaceAsync.deleteBulkPtkp(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbPtkpBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbPtkpBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbPtkpBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbPtkpBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getPtkpPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbPtkpBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbPtkpBeanModel> result) {
		...
		pagingStore = new ListStore<TbPtkpBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbptkpStatus", "<center><b>Status</b></center>", 100));
		columns.add(new ColumnConfig("tbptkpKeterangan", "<center><b>Keterangan</b></center>", 200));

		ColumnConfig columnConfig = new ColumnConfig("tbptkpJumlah", "<center><b>Jumlah</b></center>", 100);
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbptkpId").toString());
						WindowPTKPAddEdit windowPTKPAddEdit = WindowPTKPAddEdit.getInstance();
						windowPTKPAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbptkpId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete ptkp \"" + model.get("tbptkpStatus") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbPtkpBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbPtkpBeanModel>>() {
			public void handleEvent(GridEvent<TbPtkpBeanModel> be) {
