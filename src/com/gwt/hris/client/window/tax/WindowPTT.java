...
public class WindowPTT extends WindowMain {
	private static WindowPTT instance = new WindowPTT();

	public static WindowPTT getInstance() {
		return instance;
	}

	public WindowPTT() {
		window = new Window();
		window.setId("WindowPTT");
		window.setSize(600, 380);
		window.setHeading("Tax : PTT");
	}

	PTTInterfaceAsync interfaceAsync = GWT.create(PTTInterface.class);
	...
	RpcProxy<PagingLoadResult<TbPttBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbPttBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbPttBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deletePTT(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbPttBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbPttBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbPttBeanModel data = (TbPttBeanModel) itr.next();
					ids[i] = data.getTbpttId();
					i++;
				}
				interfaceAsync.deleteBulkPTT(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbPttBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbPttBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbPttBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbPttBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getPTTPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbPttBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbPttBeanModel> result) {
		...
		pagingStore = new ListStore<TbPttBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbpttPttId", "<center><b>PTT ID</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbpttId").toString());
						WindowPTTAddEdit windowPTTAddEdit = WindowPTTAddEdit.getInstance();
						windowPTTAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbpttId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete ptt \"" + model.get("tbpttId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbPttBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbPttBeanModel>>() {
			public void handleEvent(GridEvent<TbPttBeanModel> be) {
