...
public class WindowTaxRate extends WindowMain {
	private static WindowTaxRate instance = new WindowTaxRate();

	public static WindowTaxRate getInstance() {
		return instance;
	}

	public WindowTaxRate() {
		window = new Window();
		window.setId("WindowTaxRate");
	...
	RpcProxy<PagingLoadResult<TbTarifPajakBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbTarifPajakBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbTarifPajakBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteTarifPajak(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbTarifPajakBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbTarifPajakBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbTarifPajakBeanModel data = (TbTarifPajakBeanModel) itr.next();
					ids[i] = data.getTbtpId();
					i++;
				}
				interfaceAsync.deleteBulkTarifPajak(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbTarifPajakBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbTarifPajakBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbTarifPajakBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbTarifPajakBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getTarifPajakPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbTarifPajakBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbTarifPajakBeanModel> result) {
		...
		pagingStore = new ListStore<TbTarifPajakBeanModel>(pagingLoader);
		...
		ColumnConfig columnConfig = new ColumnConfig("tbtpPkpDari", "<center><b>PKP Dari</b></center>", 100);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				double data1 = model.get("tbtpPkpDari");
		...
		columnConfig = new ColumnConfig("tbtpPkpSampai", "<center><b>PKP Sampai</b></center>", 200);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				double data1 = model.get("tbtpPkpSampai");
				NumberFormat fmt = NumberFormat.getFormat("#,###,###.######");
				return fmt.format(data1);
			}
		});
		...
		columnConfig = new ColumnConfig("tbtpNpwp", "<center><b>NPWP</b></center>", 100);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				int data1 = model.get("tbtpNpwp");
				return data1 + "%";
			}
		});
		columns.add(columnConfig);
		
		columnConfig = new ColumnConfig("tbtpNonNpwp", "<center><b>Non NPWP</b></center>", 100);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				int data1 = model.get("tbtpNonNpwp");
				return data1 + "%";
			}
		});
		columns.add(columnConfig);
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbtpId").toString());
						WindowTaxRateAddEdit windowTaxRateAddEdit = WindowTaxRateAddEdit.getInstance();
						windowTaxRateAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbtpId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete tax rate \"" + model.get("tbtpPkpDari") + " - " + model.get("tbtpPkpSampai") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbTarifPajakBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbTarifPajakBeanModel>>() {
			public void handleEvent(GridEvent<TbTarifPajakBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				WindowTaxRateAddEdit windowTarifPajakAddEdit = WindowTaxRateAddEdit.getInstance();
				windowTarifPajakAddEdit.show("add", 0);
			}
		});
