...
public class WindowPTTAddEdit extends WindowMain {
	private static WindowPTTAddEdit instance = new WindowPTTAddEdit();

	public static WindowPTTAddEdit getInstance() {
		return instance;
	}

	public WindowPTTAddEdit() {
		window = new Window();
		window.setId("WindowPTTAddEdit");
	...
	AsyncCallback<TbPttBeanModel> getCallback = new AsyncCallback<TbPttBeanModel>() {
		@Override
		public void onSuccess(TbPttBeanModel result) {
			if (result.getOperationStatus()) {
				hfId.setValue(result.getTbpttId().toString());

				lblCode.setValue(result.getTbpttPttId());
				txtName.setValue(result.getTbpttName());
	...
	AsyncCallback<ReturnBean> submitCallback = new AsyncCallback<ReturnBean>() {
		...
		public void onSuccess(ReturnBean result) {
			...
			if (result.getOperationStatus()) {
				...
				TbPttBeanModel beanModel = (TbPttBeanModel) result.get("model");
				hfId.setValue(beanModel.getTbpttId().toString());
				...
				WindowPTT.getInstance().bottomToolBar.refresh();
			} else {
				MessageBox.alert("Alert", "Operation Error.<br/><br/><br/>Error Message :<br/>" + result.getMessage(), null);
			}
		}
		...
		btnSave = new Button("Save", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				if (formPanel.isValid()) {
					...
					beanModel.setTbpttId(hfId.getValue() == null ? null : Integer.parseInt(hfId.getValue()));
					beanModel.setTbpttName(StringUtil.getInstance().getString(txtName.getValue()));
