...
public class WindowEmployeePTT extends WindowMain {
	private static WindowEmployeePTT instance = new WindowEmployeePTT();

	public static WindowEmployeePTT getInstance() {
		return instance;
	}

	public WindowEmployeePTT() {
		window = new Window();
		window.setId("WindowEmployeePTT");
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewEmployeePttBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewEmployeePttBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewEmployeePttBeanModel data = (ViewEmployeePttBeanModel) itr.next();
					ids[i] = data.getTbepttId();
					i++;
				}
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<ViewEmployeePttBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<ViewEmployeePttBeanModel>();
		...
		proxy = new RpcProxy<PagingLoadResult<ViewEmployeePttBeanModel>>() {
			...
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewEmployeePttBeanModel>> callback) {
				...
				interfaceAsync.getPttPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewEmployeePttBeanModel>>() {
					@Override
		...
		columns.add(new ColumnConfig("tbpttName", "<center><b>PTT Name</b></center>", 100));
		columns.add(new ColumnConfig("tbcCurrencyId", "<center><b>Currency</b></center>", 100));
		
		ColumnConfig columnConfig = new ColumnConfig("tbepttNominal", "<center><b>Nominal</b></center>", 100);
		...
		columnConfig = new ColumnConfig("tbepttInsertTime", "<center><b>Date</b></center>", 100);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbepttId").toString());
						WindowEmployeePTTAddEdit windowPTTAddEdit = WindowEmployeePTTAddEdit.getInstance();
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbepttId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete ptt \"" + model.get("tbeName") + " - " + model.get("tbepttNominal") + "\" ?", deleteListener);
					}
				});
		...
		grid.addListener(Events.Attach, new Listener<GridEvent<ViewEmployeePttBeanModel>>() {
			public void handleEvent(GridEvent<ViewEmployeePttBeanModel> be) {
