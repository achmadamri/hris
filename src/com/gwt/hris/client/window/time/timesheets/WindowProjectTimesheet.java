...
import com.gwt.hris.client.service.time.timesheets.TimesheetProjectInterfaceAsync;
import com.gwt.hris.client.window.MainTabLayout;
import com.gwt.hris.client.window.WindowMain;

public class WindowProjectTimesheet extends WindowMain {
	private static WindowProjectTimesheet instance = new WindowProjectTimesheet();

	public static WindowProjectTimesheet getInstance() {
		return instance;
	}

	public WindowProjectTimesheet() {
		window = new Window();
		window.setId("WindowProjectTimesheet");
		window.setSize(1000, 380);
		window.setHeading("Timesheets : Project Timesheet ");
	}

	TimesheetProjectInterfaceAsync interfaceAsync = GWT.create(TimesheetProjectInterface.class);
	...
	GroupingStore<ViewTimesheetProjectBeanModel> pagingStore;
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<ViewTimesheetProjectBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<ViewTimesheetProjectBeanModel>();
		...
		proxy = new RpcProxy<PagingLoadResult<ViewTimesheetProjectBeanModel>>() {
			...
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewTimesheetProjectBeanModel>> callback) {
				...
				interfaceAsync.getTimesheetPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewTimesheetProjectBeanModel>>() {
					@Override
		...
		pagingStore = new GroupingStore<ViewTimesheetProjectBeanModel>(pagingLoader);
		pagingStore.groupBy("tbpName");
		...
		ColumnConfig columnConfig = new ColumnConfig("tbpaPayable", "<center><b>Payable</b></center>", 75);
		...
		view.setGroupRenderer(new GridGroupRenderer() {
			public String render(GroupColumnData data) {
				...
				for (int i = 0; i < lst.size(); i++) {
					lng += (Long) lst.get(i).get("sumTbtTotalHour");
				}
				return "Customer Name : " + lst.get(0).get("tbcName") + "<br>Project Name : " + data.group + "<br>Total Hour : " + lng + " hour(s)";
			}
		});
