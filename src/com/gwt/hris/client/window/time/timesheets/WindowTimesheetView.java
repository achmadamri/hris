...
public class WindowTimesheetView extends WindowMain {
	private static WindowTimesheetView instance = new WindowTimesheetView();

	public static WindowTimesheetView getInstance() {
		return instance;
	}

	public WindowTimesheetView() {
		window = new Window();
		window.setId("WindowTimesheetView");
		window.setSize(930, 350);
	...
	AsyncCallback<TbTimesheetBeanModel> getCallback = new AsyncCallback<TbTimesheetBeanModel>() {
		...
		public void onSuccess(TbTimesheetBeanModel result) {
			if (result.getOperationStatus()) {
				...
				lblStatus.setValue(result.getTbtApprovalStatus() == 0 ? "Pending" : (result.getTbtApprovalStatus() == 1 ? "Approved" : "Rejected"));
	...
	public void addComponents() {
		...
		cmbProjectActivities.setAllowBlank(false);cmbProjectActivities.addListener(Events.Select, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {
