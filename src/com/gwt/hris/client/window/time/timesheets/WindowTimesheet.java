...
public class WindowTimesheet extends WindowMain {
	private static WindowTimesheet instance = new WindowTimesheet();

	public static WindowTimesheet getInstance() {
		return instance;
	}

	public WindowTimesheet() {
		window = new Window();
		window.setId("WindowTimesheet");
		window.setSize(900, 380);
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteTimesheet(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewTimesheetBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewTimesheetBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewTimesheetBeanModel data = (ViewTimesheetBeanModel) itr.next();
					ids[i] = data.getTbtId();
					i++;
				}
				interfaceAsync.deleteBulkTimesheet(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		proxy = new RpcProxy<PagingLoadResult<ViewTimesheetBeanModel>>() {
			...
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewTimesheetBeanModel>> callback) {
				...
				interfaceAsync.getTimesheetPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewTimesheetBeanModel>>() {
					@Override
		...
		pagingStore.groupBy("tbtStartOfWeek");

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbcName", "<center><b>Customer</b></center>", 150));

		ColumnConfig columnConfig = new ColumnConfig("tbpName", "<center><b>Project</b></center>", 150);
		...
		columnConfig = new ColumnConfig("totalHour", "<center><b>Total</b></center>", 75);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				if (data == 0) {
					...
					Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
						...
						public void componentSelected(ButtonEvent ce) {
							...
							WindowTimesheetAddEdit windowTimesheetAddEdit = WindowTimesheetAddEdit.getInstance();
							windowTimesheetAddEdit.show("edit", id);
						}
					});
					...
					Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
						@Override
						public void componentSelected(ButtonEvent ce) {
							deleteID = Integer.parseInt(model.get("tbtId").toString());

							MessageBox.confirm("Confirmation", "Are you sure you want to delete timesheet \"" + model.get("tbtStartOfWeek") + "\" ?", deleteListener);
						}
					});
					...
					Button btnView = new Button("", new SelectionListener<ButtonEvent>() {
						...
						public void componentSelected(ButtonEvent ce) {
							int id = Integer.parseInt(model.get("tbtId").toString());
							WindowTimesheetView windowTimesheetView = WindowTimesheetView.getInstance();
							windowTimesheetView.show(id);
						}
					});
		...
		view.setGroupRenderer(new GridGroupRenderer() {
			public String render(GroupColumnData data) {
				...
				return "Start Of Week : " + data.group + "<br>Total Hour : " + lst.get(0).get("tbtGrandTotalHour") + " hour(s)";
			}
		});
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowTimesheetAddEdit windowTimesheetAddEdit = WindowTimesheetAddEdit.getInstance();
				windowTimesheetAddEdit.show("add", 0);
			}
		});
