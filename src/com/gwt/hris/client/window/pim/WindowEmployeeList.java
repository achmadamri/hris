...
public class WindowEmployeeList extends WindowMain {
	private static WindowEmployeeList instance = new WindowEmployeeList();

	public static WindowEmployeeList getInstance() {
		return instance;
	}

	public WindowEmployeeList() {
		window = new Window();
		window.setId("WindowEmployeeList");
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.disableEmployee(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewEmployeeInformationBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewEmployeeInformationBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewEmployeeInformationBeanModel data = (ViewEmployeeInformationBeanModel) itr.next();
					ids[i] = data.getTbeId();
					i++;
				}
				interfaceAsync.disableBulkEmployee(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<ViewEmployeeInformationBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<ViewEmployeeInformationBeanModel>();
		...
		columns.add(new ColumnConfig("tboNama", "<center><b>Organization</b></center>", 75));
		...
		columnConfig = new ColumnConfig("tbeJoinedDate", "<center><b>Joined Date</b></center>", 100);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				Date data = model.get("tbeJoinedDate");
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowEmployeeListEss windowEmployeeListEss = WindowEmployeeListEss.getInstance();
						windowEmployeeListEss.show(id, 1);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						MessageBox.confirm("Confirmation", "Are you sure you want to delete employee \"" + model.get("tbeName") + "\" ?", deleteListener);
					}
				});
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				WindowEmployeeListAddEdit windowEmployeeListAddEdit = WindowEmployeeListAddEdit.getInstance();
				windowEmployeeListAddEdit.show("add", 0);
			}
		});
