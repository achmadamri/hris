...
import com.gwt.hris.client.service.pim.PphInterfaceAsync;
import com.gwt.hris.client.window.MainTabLayout;

public class EssPPH extends EssMainCP {
	FormPanel formPanel;

	public EssPPH(FormData formData, int id_, int callerId_) {
		super(formData);
	...
	PphInterfaceAsync PphInterfaceAsync = GWT.create(PphInterface.class);

	public int id = 0;
	public int callerId;

	RpcProxy<PagingLoadResult<TbPphBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbPphBeanModel> pagingStore;
	PagingToolBar bottomToolBar;
	Grid<TbPphBeanModel> grid;
	...
	public void addComponents() {
		...
		proxy = new RpcProxy<PagingLoadResult<TbPphBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbPphBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				PphInterfaceAsync.getPphPaging(pagingLoadConfig, "tbe_id", String.valueOf(id), new AsyncCallback<PagingLoadResult<TbPphBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbPphBeanModel> result) {
		...
		pagingStore = new ListStore<TbPphBeanModel>(pagingLoader);
		...
		columnConfig = new ColumnConfig("tbpphTotalTakeHomePay", "<center><b>Total THP</b></center>", 100);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnReport = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						int pphId = Integer.parseInt(model.get("tbpphId").toString());

						String url = GWT.getHostPageBaseURL() + "report/?name=pph&tbeId=" + id + "&tbpphId=" + pphId;

						com.google.gwt.user.client.Window.open(url, "", "");
					}
				});
				btnReport.setToolTip("view pay slip");
				btnReport.setIcon(Resources.ICONS.pdf16());
		...
		grid = new Grid<TbPphBeanModel>(pagingStore, cm);
