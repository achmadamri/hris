...
public class WindowEss extends WindowMain {
	private static WindowEss instance = new WindowEss();

	public static WindowEss getInstance() {
		return instance;
	}

	public WindowEss() {
		window = new Window();
		window.setId("WindowEss");
		window.setSize(740, 550);
		window.setHeading("ESS : ESS");
	}
	...
	AsyncCallback<TbEmployeeBeanModel> getCallback = new AsyncCallback<TbEmployeeBeanModel>() {
		...
		public void onSuccess(TbEmployeeBeanModel result) {
			if (result.getOperationStatus()) {
				...
				if (WindowEss.getInstance().callerId == 0) {
					show(false);
				} else {
					show(true);
				}
	...
	public void addComponents() {
		...
		// Start tabItemInformation
		TabItem tabItemInformation = new TabItem("Information");
		tabItemInformation.setStyleAttribute("backgroundColor", "#F2F2F2");
		tabItemInformation.setLayout(new FillLayout());

		TabPanel tabPanelInformation = new TabPanel();
		tabPanelInformation.setTabScroll(true);

		final TabItem tabItemNotification = new TabItem("Notification");
		tabItemNotification.setStyleAttribute("backgroundColor", "#F2F2F2");
		tabItemNotification.setLayout(new FillLayout());
		tabItemNotification.addListener(Events.Select, new Listener<ComponentEvent>() {
			public void handleEvent(ComponentEvent be) {
				tabItemNotification.removeAll();
				tabItemNotification.add(new EssNotification(formData, id, callerId));
				window.layout(true);
			}
		});
		tabPanelInformation.add(tabItemNotification);

		tabItemInformation.add(tabPanelInformation);

		tabPanel.add(tabItemInformation);
		// End tabItemInformation
