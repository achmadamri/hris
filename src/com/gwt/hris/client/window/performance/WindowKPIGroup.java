...
public class WindowKPIGroup extends WindowMain {
	private static WindowKPIGroup instance = new WindowKPIGroup();

	public static WindowKPIGroup getInstance() {
		return instance;
	}

	public WindowKPIGroup() {
		window = new Window();
		window.setId("WindowKpiGroup");
	...
	RpcProxy<PagingLoadResult<TbKpiGroupBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbKpiGroupBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbKpiGroupBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteKpiGroup(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbKpiGroupBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbKpiGroupBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbKpiGroupBeanModel data = (TbKpiGroupBeanModel) itr.next();
					ids[i] = data.getTbkgId();
					i++;
				}
				interfaceAsync.deleteBulkKpiGroup(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbKpiGroupBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbKpiGroupBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbKpiGroupBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbKpiGroupBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getKpiGroupPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbKpiGroupBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbKpiGroupBeanModel> result) {
		...
		pagingStore = new ListStore<TbKpiGroupBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbkgKpiGroupId", "<center><b>KPI Group ID</b></center>", 100));
		columns.add(new ColumnConfig("tbkgName", "<center><b>KPI Group</b></center>", 200));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbkgId").toString());
						WindowKPIGroupAddEdit windowKpiGroupAddEdit = WindowKPIGroupAddEdit.getInstance();
						windowKpiGroupAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbkgId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete kpi group \"" + model.get("tbkgKpiGroupId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbKpiGroupBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbKpiGroupBeanModel>>() {
			public void handleEvent(GridEvent<TbKpiGroupBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowKPIGroupAddEdit windowKpiGroupAddEdit = WindowKPIGroupAddEdit.getInstance();
				windowKpiGroupAddEdit.show("add", 0);
			}
		});
