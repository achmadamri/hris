...
public class WindowKPIAssign extends WindowMain {
	private static WindowKPIAssign instance = new WindowKPIAssign();

	NumberFormat number = NumberFormat.getFormat("0.00");

	public static WindowKPIAssign getInstance() {
		return instance;
	}

	public WindowKPIAssign() {
		window = new Window();
		window.setId("WindowKPIAssign");
	...
	AsyncCallback<TbEmployeeBeanModel> getTbEmployeeAssignCallback = new AsyncCallback<TbEmployeeBeanModel>() {
		@Override
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteKpiAssign(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewKpiAssignBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewKpiAssignBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewKpiAssignBeanModel data = (ViewKpiAssignBeanModel) itr.next();
					ids[i] = data.getTbkaId();
					i++;
				}
				interfaceAsync.deleteBulkKpiAssign(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		kpiAssignInterfaceAsync.getTbEmployeeBeanAssign(getTbEmployeeAssignCallback);
		...
		proxy = new RpcProxy<PagingLoadResult<ViewKpiAssignBeanModel>>() {
			...
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewKpiAssignBeanModel>> callback) {
				...
				interfaceAsync.getKpiAssignPaging(pagingLoadConfig, cmbEmployee.getSimpleValue(), cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewKpiAssignBeanModel>>() {
					@Override
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				if (data == 0 || data == 2 || data == 5) {
					...
					Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
						@Override
						public void componentSelected(ButtonEvent ce) {
							deleteID = Integer.parseInt(model.get("tbkaId").toString());

							MessageBox.confirm("Confirmation", "Are you sure you want to delete kpi " + model.get("tbkaId").toString() + " assign to " + model.get("tbeName").toString() + " ?", deleteListener);
						}
					});
					...
					Button btnScoring = new Button("", new SelectionListener<ButtonEvent>() {
						...
						public void componentSelected(ButtonEvent ce) {
							...
							WindowKPIAssignScoring windowKPIAssignScoring = WindowKPIAssignScoring.getInstance();
							windowKPIAssignScoring.show("edit", id);
						}
					});
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				WindowKPIAssignAddEdit windowKPIAssignAdd = WindowKPIAssignAddEdit.getInstance();
				windowKPIAssignAdd.show("add", 0);
			}
		});
