...
public class WindowKPI extends WindowMain {
	private static WindowKPI instance = new WindowKPI();

	public static WindowKPI getInstance() {
		return instance;
	}

	public WindowKPI() {
		window = new Window();
		window.setId("WindowKPI");
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteKpi(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				...
				interfaceAsync.deleteBulkKpi(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		proxy = new RpcProxy<PagingLoadResult<ViewKpiBeanModel>>() {
			...
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewKpiBeanModel>> callback) {
				...
				interfaceAsync.getKpiPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewKpiBeanModel>>() {
					@Override
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbkId").toString());
						WindowKPIAddEdit windowKPIAddEdit = WindowKPIAddEdit.getInstance();
						windowKPIAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbkId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete kpi ?", deleteListener);
					}
				});
