...
public class WindowKPIApprovalScoring extends WindowMain {
	private static WindowKPIApprovalScoring instance = new WindowKPIApprovalScoring();

	NumberFormat number = NumberFormat.getFormat("0.00");

	public static WindowKPIApprovalScoring getInstance() {
		return instance;
	}

	public WindowKPIApprovalScoring() {
		window = new Window();
		window.setId("WindowKPIApprovalScoring");
		window.setSize(980, 380);
		window.setHeading("Performance : KPI Approval Scoring");
	}

	KPIAssignInterfaceAsync interfaceAsync = GWT.create(KPIAssignInterface.class);
	SimpleComboBox<String> cmbEmployee;
	...
	AsyncCallback<TbEmployeeBeanModel> getTbEmployeeApprovalScoringCallback = new AsyncCallback<TbEmployeeBeanModel>() {
		@Override
		public void onSuccess(TbEmployeeBeanModel result) {
			if (result.getOperationStatus()) {
				...
				for (TbEmployeeBeanModel obj : result.getModels()) {
					cmbEmployee.add(obj.getTbeName());
				}
			} else {
	...
	Listener<MessageBoxEvent> approveListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.approveKpiAssign(id, 1, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> rejectListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.approveKpiAssign(id, 2, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		toolBar.add(new LabelToolItem("Employee Name : "));

		cmbEmployee = new SimpleComboBox<String>();
		cmbEmployee.setTriggerAction(TriggerAction.ALL);
		cmbEmployee.setEditable(false);
		cmbEmployee.setWidth(110);
		cmbEmployee.addListener(Events.Select, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {
				bottomToolBar.refresh();
			}
		});
		toolBar.add(cmbEmployee);
		kpiAssignInterfaceAsync.getTbEmployeeBeanApprovalScoring(getTbEmployeeApprovalScoringCallback);
		...
		proxy = new RpcProxy<PagingLoadResult<ViewKpiAssignBeanModel>>() {
			...
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewKpiAssignBeanModel>> callback) {
				...
				interfaceAsync.getKpiApprovalScoringPaging(pagingLoadConfig, cmbEmployee.getSimpleValue(), cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewKpiAssignBeanModel>>() {
					@Override
		...
		columns.add(new ColumnConfig("tbkaPoin", "<center><b>Poin</b></center>", 100));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				if (data == 3) {
					...
					Button btnApprove = new Button("", new SelectionListener<ButtonEvent>() {
						@Override
						public void componentSelected(ButtonEvent ce) {
							id = Integer.parseInt(model.get("tbkaId").toString());

							MessageBox.confirm("Confirmation", "Are you sure you want to approve kpi " + model.get("tbkaId").toString() + " assign to " + model.get("tbeName").toString() + " ?", approveListener);
						}
					});
					btnApprove.setToolTip("approve");
					btnApprove.setIcon(Resources.ICONS.ok());
					layoutContainer.add(btnApprove, hBoxLayoutData);

					Button btnReject = new Button("", new SelectionListener<ButtonEvent>() {
						@Override
						public void componentSelected(ButtonEvent ce) {
							id = Integer.parseInt(model.get("tbkaId").toString());

							MessageBox.confirm("Confirmation", "Are you sure you want to reject kpi " + model.get("tbkaId").toString() + " assign to " + model.get("tbeName").toString() + " ?", rejectListener);
						}
					});
					btnReject.setToolTip("reject");
					btnReject.setIcon(Resources.ICONS.cancel());
					layoutContainer.add(btnReject, hBoxLayoutData);
