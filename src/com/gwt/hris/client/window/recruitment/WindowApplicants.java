...
public class WindowApplicants extends WindowMain {
	private static WindowApplicants instance = new WindowApplicants();

	public static WindowApplicants getInstance() {
		return instance;
	}

	public WindowApplicants() {
		window = new Window();
		window.setId("WindowApplicants");
	...
	RpcProxy<PagingLoadResult<ViewApplicantsBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<ViewApplicantsBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<ViewApplicantsBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteApplicants(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewApplicantsBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewApplicantsBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewApplicantsBeanModel data = (ViewApplicantsBeanModel) itr.next();
					ids[i] = data.getTbaId();
					i++;
				}
				interfaceAsync.deleteBulkApplicants(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("App. ID");
		cmbSearch.add("Candidate");
		cmbSearch.add("Hiring Manager");
		cmbSearch.add("Submit Date");
		...
		CheckBoxSelectionModel<ViewApplicantsBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<ViewApplicantsBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<ViewApplicantsBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewApplicantsBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getApplicantsPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewApplicantsBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<ViewApplicantsBeanModel> result) {
		...
		pagingStore = new ListStore<ViewApplicantsBeanModel>(pagingLoader);
		...
		columns.add(new ColumnConfig("tbaApplicantsId", "<center><b>App. ID</b></center>", 75));
		columns.add(new ColumnConfig("tbaName", "<center><b>Candidate</b></center>", 150));
		columns.add(new ColumnConfig("tbeName", "<center><b>Hiring Manager</b></center>", 150));

		ColumnConfig columnConfig = new ColumnConfig("tbaDate", "<center><b>Submit Date</b></center>", 100);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				Date data = model.get("tbaDate");
		...
		columnConfig.setHeader("<center><b>Resume</b></center>");
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnReport = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						String strTbeResumeFileName = model.get("tbaResumeFileName").toString();

						String url = GWT.getHostPageBaseURL() + "upload/resume/" + strTbeResumeFileName;

						com.google.gwt.user.client.Window.open(url, "", "");
					}
				});
				String strTbaResumeFileNameExisting = model.get("tbaResumeFileNameExisting").toString();
				btnReport.setToolTip(strTbaResumeFileNameExisting);
				btnReport.setIcon(Resources.ICONS.text());
				layoutContainer.add(btnReport, hBoxLayoutData);
		...
		columnConfig = new ColumnConfig("tbaStatus", "<center><b>Status</b></center>", 75);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				Integer data = model.get("tbaStatus");
				return (data == 0 ? "Initiated" : (data == 1 ? "Shortlist" : "Rejected"));
			}
		});
		columns.add(columnConfig);
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbaId").toString());
						WindowApplicantsAddEdit window = WindowApplicantsAddEdit.getInstance();
						window.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbaId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete applicant \"" + model.get("tbaName") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<ViewApplicantsBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<ViewApplicantsBeanModel>>() {
			public void handleEvent(GridEvent<ViewApplicantsBeanModel> be) {
