...
public class WindowSkills extends WindowMain {
	private static WindowSkills instance = new WindowSkills();

	public static WindowSkills getInstance() {
		return instance;
	}

	public WindowSkills() {
		window = new Window();
		window.setId("WindowSkills");
	...
	RpcProxy<PagingLoadResult<TbSkillsBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbSkillsBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbSkillsBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteSkills(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbSkillsBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbSkillsBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbSkillsBeanModel data = (TbSkillsBeanModel) itr.next();
					ids[i] = data.getTbsId();
					i++;
				}
				interfaceAsync.deleteBulkSkills(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Skill ID");
		cmbSearch.add("Skill Name");
		...
		CheckBoxSelectionModel<TbSkillsBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbSkillsBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbSkillsBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbSkillsBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getSkillsPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbSkillsBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbSkillsBeanModel> result) {
		...
		pagingStore = new ListStore<TbSkillsBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbsSkillId", "<center><b>Skills ID</b></center>", 150));
		columns.add(new ColumnConfig("tbsName", "<center><b>Skills Name</b></center>", 150));
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowSkillsAddEdit windowSkillsAddEdit = WindowSkillsAddEdit.getInstance();
						windowSkillsAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						MessageBox.confirm("Confirmation", "Are you sure you want to delete skill \"" + model.get("tbsSkillId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbSkillsBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbSkillsBeanModel>>() {
			public void handleEvent(GridEvent<TbSkillsBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowSkillsAddEdit windowSkillsAddEdit = WindowSkillsAddEdit.getInstance();
				windowSkillsAddEdit.show("add", 0);
			}
		});
