...
public class WindowShift extends WindowMain {
	private static WindowShift instance = new WindowShift();

	public static WindowShift getInstance() {
		return instance;
	}

	public WindowShift() {
		window = new Window();
		window.setId("WindowShift");
	...
	RpcProxy<PagingLoadResult<TbShiftBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbShiftBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbShiftBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteShift(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbShiftBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbShiftBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbShiftBeanModel data = (TbShiftBeanModel) itr.next();
					ids[i] = data.getTbsId();
					i++;
				}
				interfaceAsync.deleteBulkShift(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbShiftBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbShiftBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbShiftBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbShiftBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getShiftPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbShiftBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbShiftBeanModel> result) {
		...
		pagingStore = new ListStore<TbShiftBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbsShiftId", "<center><b>Shift ID</b></center>", 150));
		columns.add(new ColumnConfig("tbsName", "<center><b>Shift Name</b></center>", 100));
		columns.add(new ColumnConfig("tbsInTime", "<center><b>In Time</b></center>", 100));
		columns.add(new ColumnConfig("tbsOutTime", "<center><b>Out Time</b></center>", 100));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbsId").toString());
						WindowShiftAddEdit windowShiftAddEdit = WindowShiftAddEdit.getInstance();
						windowShiftAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbsId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete shift \"" + model.get("tbsShiftId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbShiftBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbShiftBeanModel>>() {
			public void handleEvent(GridEvent<TbShiftBeanModel> be) {
