...
public class WindowNationality extends WindowMain {
	private static WindowNationality instance = new WindowNationality();

	public static WindowNationality getInstance() {
		return instance;
	}

	public WindowNationality() {
		window = new Window();
		window.setId("WindowNationality");
	...
	RpcProxy<PagingLoadResult<TbNationalityBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbNationalityBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbNationalityBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteNationality(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbNationalityBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbNationalityBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbNationalityBeanModel data = (TbNationalityBeanModel) itr.next();
					ids[i] = data.getTbnId();
					i++;
				}
				interfaceAsync.deleteBulkNationality(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Nationality ID");
		cmbSearch.add("Nationality Name");
		...
		CheckBoxSelectionModel<TbNationalityBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbNationalityBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbNationalityBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbNationalityBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getNationalityPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbNationalityBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbNationalityBeanModel> result) {
		...
		pagingStore = new ListStore<TbNationalityBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbnNationalityId", "<center><b>Nationality ID</b></center>", 150));
		columns.add(new ColumnConfig("tbnName", "<center><b>Nationality Name</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbnId").toString());
						WindowNationalityAddEdit windowNationalityAddEdit = WindowNationalityAddEdit.getInstance();
						windowNationalityAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbnId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete nationality \"" + model.get("tbnNationalityId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbNationalityBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbNationalityBeanModel>>() {
			public void handleEvent(GridEvent<TbNationalityBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowNationalityAddEdit windowNationalityAddEdit = WindowNationalityAddEdit.getInstance();
				windowNationalityAddEdit.show("add", 0);
			}
		});
