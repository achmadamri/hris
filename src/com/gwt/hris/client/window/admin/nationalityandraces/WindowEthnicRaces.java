...
public class WindowEthnicRaces extends WindowMain {
	private static WindowEthnicRaces instance = new WindowEthnicRaces();

	public static WindowEthnicRaces getInstance() {
		return instance;
	}

	public WindowEthnicRaces() {
		window = new Window();
		window.setId("WindowEthnicRaces");
		window.setSize(610, 380);
		window.setHeading("Nationality & Race : Ethnic Races");
	}

	EthnicInterfaceAsync interfaceAsync = GWT.create(EthnicInterface.class);
	...
	RpcProxy<PagingLoadResult<TbEthnicRacesBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbEthnicRacesBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbEthnicRacesBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteEthnic(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbEthnicRacesBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbEthnicRacesBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbEthnicRacesBeanModel data = (TbEthnicRacesBeanModel) itr.next();
					ids[i] = data.getTberId();
					i++;
				}
				interfaceAsync.deleteBulkEthnic(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Ethnic Race ID");
		cmbSearch.add("Ethnic Race Name");
		...
		CheckBoxSelectionModel<TbEthnicRacesBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbEthnicRacesBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbEthnicRacesBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbEthnicRacesBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getEthnicPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbEthnicRacesBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbEthnicRacesBeanModel> result) {
		...
		pagingStore = new ListStore<TbEthnicRacesBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tberEthnicRacesId", "<center><b>Ethnic Races ID</b></center>", 150));
		columns.add(new ColumnConfig("tberName", "<center><b>Ethnic Race Name</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tberId").toString());
						WindowEthnicRacesAddEdit windowEthnicRacesAddEdit = WindowEthnicRacesAddEdit.getInstance();
						windowEthnicRacesAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tberId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete ethnic races \"" + model.get("tberEthnicRacesId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbEthnicRacesBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbEthnicRacesBeanModel>>() {
			public void handleEvent(GridEvent<TbEthnicRacesBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowEthnicRacesAddEdit windowEthnicRacesAddEdit = WindowEthnicRacesAddEdit.getInstance();
				windowEthnicRacesAddEdit.show("add", 0);
			}
		});
