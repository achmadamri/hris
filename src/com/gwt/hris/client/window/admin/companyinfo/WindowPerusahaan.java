...
public class WindowPerusahaan extends WindowMain {
	private static WindowPerusahaan instance = new WindowPerusahaan();

	public static WindowPerusahaan getInstance() {
		return instance;
	}

	public WindowPerusahaan() {
		window = new Window();
		window.setId("WindowPerusahaan");
	...
	RpcProxy<PagingLoadResult<TbPerusahaanBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbPerusahaanBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbPerusahaanBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				PerusahaanInterfaceAsync.deletePerusahaan(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbPerusahaanBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbPerusahaanBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbPerusahaanBeanModel data = (TbPerusahaanBeanModel) itr.next();
					ids[i] = data.getTbpId();
					i++;
				}
				PerusahaanInterfaceAsync.deleteBulkPerusahaan(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbPerusahaanBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbPerusahaanBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbPerusahaanBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbPerusahaanBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				PerusahaanInterfaceAsync.getTbPerusahaanPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbPerusahaanBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbPerusahaanBeanModel> result) {
		...
		pagingStore = new ListStore<TbPerusahaanBeanModel>(pagingLoader);
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowPerusahaanAddEdit windowPerusahaanAdd = WindowPerusahaanAddEdit.getInstance();
						windowPerusahaanAdd.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						MessageBox.confirm("Confirmation", "Are you sure you want to delete Company \"" + model.get("tbpName") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbPerusahaanBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbPerusahaanBeanModel>>() {
			public void handleEvent(GridEvent<TbPerusahaanBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowPerusahaanAddEdit windowPerusahaanAdd = WindowPerusahaanAddEdit.getInstance();
				windowPerusahaanAdd.show("add", 0);
			}
		});
