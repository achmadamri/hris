...
public class WindowNotificationTemplate extends WindowMain {
	private static WindowNotificationTemplate instance = new WindowNotificationTemplate();

	public static WindowNotificationTemplate getInstance() {
		return instance;
	}

	public WindowNotificationTemplate() {
		window = new Window();
		window.setId("WindowNotificationTemplate");
	...
	RpcProxy<PagingLoadResult<TbNotificationTemplateBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbNotificationTemplateBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbNotificationTemplateBeanModel> grid;
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbNotificationTemplateBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbNotificationTemplateBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbNotificationTemplateBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbNotificationTemplateBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getNotificationTemplatePaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbNotificationTemplateBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbNotificationTemplateBeanModel> result) {
		...
		pagingStore = new ListStore<TbNotificationTemplateBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbntSubject", "<center><b>Subject</b></center>", 200));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbntId").toString());
						WindowNotificationTemplateAddEdit windowNotificationAddEdit = WindowNotificationTemplateAddEdit.getInstance();
						windowNotificationAddEdit.show("edit", id);
					}
				});
		...
		grid = new Grid<TbNotificationTemplateBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbNotificationTemplateBeanModel>>() {
			public void handleEvent(GridEvent<TbNotificationTemplateBeanModel> be) {
