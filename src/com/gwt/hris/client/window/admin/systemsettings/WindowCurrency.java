...
public class WindowCurrency extends WindowMain {
	private static WindowCurrency instance = new WindowCurrency();

	public static WindowCurrency getInstance() {
		return instance;
	}

	public WindowCurrency() {
		window = new Window();
		window.setId("WindowCurrency");
	...
	RpcProxy<PagingLoadResult<TbCurrencyBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbCurrencyBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbCurrencyBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteCurrency(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbCurrencyBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbCurrencyBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbCurrencyBeanModel data = (TbCurrencyBeanModel) itr.next();
					ids[i] = data.getTbcId();
					i++;
				}
				interfaceAsync.deleteBulkCurrency(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Currency ID");
		cmbSearch.add("Currency Name");
		...
		CheckBoxSelectionModel<TbCurrencyBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbCurrencyBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbCurrencyBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbCurrencyBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getCurrencyPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbCurrencyBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbCurrencyBeanModel> result) {
		...
		pagingStore = new ListStore<TbCurrencyBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbcCurrencyId", "<center><b>Currency ID</b></center>", 75));
		columns.add(new ColumnConfig("tbcName", "<center><b>Currency Name</b></center>", 200));

		ColumnConfig columnConfig = new ColumnConfig("tbcLocalCurrencyKurs", "<center><b>Kurs</b></center>", 100);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				double data1 = model.get("tbcLocalCurrencyKurs");
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowCurrencyAddEdit windowCurrencyAddEdit = WindowCurrencyAddEdit.getInstance();
						windowCurrencyAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						MessageBox.confirm("Confirmation", "Are you sure you want to delete currency \"" + model.get("tbcName") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbCurrencyBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbCurrencyBeanModel>>() {
			public void handleEvent(GridEvent<TbCurrencyBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowCurrencyAddEdit windowCurrencyAddEdit = WindowCurrencyAddEdit.getInstance();
				windowCurrencyAddEdit.show("add", 0);
			}
		});
