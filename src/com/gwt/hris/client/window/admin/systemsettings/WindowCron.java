...
public class WindowCron extends WindowMain {
	private static WindowCron instance = new WindowCron();

	public static WindowCron getInstance() {
		return instance;
	}

	public WindowCron() {
		window = new Window();
		window.setId("WindowCron");
	...
	RpcProxy<PagingLoadResult<TbCronBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbCronBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbCronBeanModel> grid;

	int id;

	Listener<MessageBoxEvent> runManualListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			String strBtnText = ce.getButtonClicked().getText();

			if ("Yes".equals(strBtnText)) {
				interfaceAsync.runManualCron(id, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteCron(id, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbCronBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbCronBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbCronBeanModel data = (TbCronBeanModel) itr.next();
					ids[i] = data.getTbcId();
					i++;
				}
				interfaceAsync.deleteBulkCron(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbCronBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbCronBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbCronBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbCronBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getCronPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbCronBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbCronBeanModel> result) {
		...
		pagingStore = new ListStore<TbCronBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbcName", "<center><b>Name</b></center>", 150));
		columns.add(new ColumnConfig("tbcClass", "<center><b>Class</b></center>", 200));
		columns.add(new ColumnConfig("tbcCronExp", "<center><b>Cron Expression</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbcId").toString());
						WindowCronAddEdit windowCronAddEdit = WindowCronAddEdit.getInstance();
						windowCronAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						id = Integer.parseInt(model.get("tbcId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete Cron \"" + model.get("tbcName") + "\" ?", deleteListener);
					}
				});
				...
				Button btnExecute = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						id = Integer.parseInt(model.get("tbcId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to run manual?", runManualListener);
					}
				});
				btnExecute.setToolTip("run manual");
				btnExecute.setIcon(Resources.ICONS.connect());
				layoutContainer.add(btnExecute, hBoxLayoutData);
		...
		grid = new Grid<TbCronBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbCronBeanModel>>() {
			public void handleEvent(GridEvent<TbCronBeanModel> be) {
