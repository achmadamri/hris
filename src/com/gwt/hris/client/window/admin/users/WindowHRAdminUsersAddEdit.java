...
public class WindowHRAdminUsersAddEdit extends WindowMain {
	private static WindowHRAdminUsersAddEdit instance = new WindowHRAdminUsersAddEdit();

	public static WindowHRAdminUsersAddEdit getInstance() {
		return instance;
	}

	public WindowHRAdminUsersAddEdit() {
		window = new Window();
		window.setId("WindowHRAdminUsersAddEdit");
		window.setSize(500, 270);
		window.setHeading("Users : HR Admin Users");
	}
	...
	ComboBox<ComboBoxData> cmbAdminUserGroups = new ComboBox<ComboBoxData>();
	ComboBoxData cmbAdminUserGroupsSelectedData;
	ListStore<ComboBoxData> cmbAdminUserGroupsStore = new ListStore<ComboBoxData>();
	HiddenField<String> hfAdminUserGroupsId = new HiddenField<String>();
	...
	AsyncCallback<TbAdminUserGroupsBeanModel> tbAdminUserGroupsAllCallback = new AsyncCallback<TbAdminUserGroupsBeanModel>() {
		@Override
		public void onSuccess(TbAdminUserGroupsBeanModel result) {
			if (result.getOperationStatus()) {
				cmbAdminUserGroupsStore.removeAll();

				for (TbAdminUserGroupsBeanModel obj : result.getModels()) {
					ComboBoxData data = new ComboBoxData(obj.getTbaugId().toString(), obj.getTbaugName());
					cmbAdminUserGroupsStore.add(data);

					if (hfAdminUserGroupsId.getValue() != null) {
						if (!"".equals(hfAdminUserGroupsId.getValue())) {
							if (obj.getTbaugId() == Integer.parseInt(hfAdminUserGroupsId.getValue())) {
								cmbAdminUserGroupsSelectedData = data;
								cmbAdminUserGroups.setValue(cmbAdminUserGroupsSelectedData);
							}
						}
					}
				}
	...
	AsyncCallback<TbLoginBeanModel> getCallback = new AsyncCallback<TbLoginBeanModel>() {
		...
		public void onSuccess(TbLoginBeanModel result) {
			if (result.getOperationStatus()) {
				...
				hfAdminUserGroupsId.setValue(result.getTbaugId().toString());
				hfStatusId.setValue(result.getTblStatus().toString());

				interfaceAsync.getTbAdminUserGroupsAll(tbAdminUserGroupsAllCallback);
	...
	AsyncCallback<ReturnBean> submitCallback = new AsyncCallback<ReturnBean>() {
		...
		public void onSuccess(ReturnBean result) {
			...
			if (result.getOperationStatus()) {
				...
				WindowHRAdminUsers.getInstance().bottomToolBar.refresh();
			} else {
				MessageBox.alert("Alert", "Operation Error.<br/><br/><br/>Error Message :<br/>" + result.getMessage(), null);
			}
		}
	...
	public void addComponents() {
		...
		cmbAdminUserGroups.setId("cmbAdminUserGroups");
		cmbAdminUserGroups.setFieldLabel("Admin User Groups *");
		cmbAdminUserGroups.setEmptyText("Select");
		cmbAdminUserGroups.setDisplayField("value");
		cmbAdminUserGroups.setForceSelection(true);
		cmbAdminUserGroups.setTypeAhead(true);
		cmbAdminUserGroups.setTriggerAction(TriggerAction.ALL);
		cmbAdminUserGroups.setStore(cmbAdminUserGroupsStore);
		cmbAdminUserGroups.setAllowBlank(false);
		formPanel.add(cmbAdminUserGroups, formData);

		formPanel.add(hfAdminUserGroupsId, formData);
		...
		btnSave = new Button("Save", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				if (formPanel.isValid()) {
					...
					beanModel.setTbaugId(Integer.parseInt(cmbAdminUserGroups.getValue().get("id").toString()));
