...
public class WindowAdminUserGroups extends WindowMain {
	private static WindowAdminUserGroups instance = new WindowAdminUserGroups();

	public static WindowAdminUserGroups getInstance() {
		return instance;
	}

	public WindowAdminUserGroups() {
		window = new Window();
		window.setId("WindowAdminUserGroups");
	...
	RpcProxy<PagingLoadResult<TbAdminUserGroupsBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbAdminUserGroupsBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbAdminUserGroupsBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteAdminUserGroups(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbAdminUserGroupsBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbAdminUserGroupsBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbAdminUserGroupsBeanModel data = (TbAdminUserGroupsBeanModel) itr.next();
					ids[i] = data.getTbaugId();
					i++;
				}
				interfaceAsync.deleteBulkAdminUserGroups(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("User Group ID");
		cmbSearch.add("User Group Name");
		...
		CheckBoxSelectionModel<TbAdminUserGroupsBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbAdminUserGroupsBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbAdminUserGroupsBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbAdminUserGroupsBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getAdminUserGroupsPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbAdminUserGroupsBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbAdminUserGroupsBeanModel> result) {
		...
		pagingStore = new ListStore<TbAdminUserGroupsBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbaugAdminUserGroupsId", "<center><b>User Group ID</b></center>", 150));
		columns.add(new ColumnConfig("tbaugName", "<center><b>User Group Name</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbaugId").toString());
						WindowAdminUserGroupsAddEdit windowAdminUserGroupsAddEdit = WindowAdminUserGroupsAddEdit.getInstance();
						windowAdminUserGroupsAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbaugId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete admin user groups \"" + model.get("tbaugAdminUserGroupsId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbAdminUserGroupsBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbAdminUserGroupsBeanModel>>() {
			public void handleEvent(GridEvent<TbAdminUserGroupsBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowAdminUserGroupsAddEdit windowAdminUserGroupsAddEdit = WindowAdminUserGroupsAddEdit.getInstance();
				windowAdminUserGroupsAddEdit.show("add", 0);
			}
		});
