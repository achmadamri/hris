...
public class WindowHRAdminUsers extends WindowMain {
	private static WindowHRAdminUsers instance = new WindowHRAdminUsers();

	public static WindowHRAdminUsers getInstance() {
		return instance;
	}

	public WindowHRAdminUsers() {
		window = new Window();
		window.setId("WindowHRAdminUsers");
	...
	RpcProxy<PagingLoadResult<TbLoginBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbLoginBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbLoginBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteLogin(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbLoginBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbLoginBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbLoginBeanModel data = (TbLoginBeanModel) itr.next();
					ids[i] = data.getTblId();
					i++;
				}
				interfaceAsync.deleteBulkLogin(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("User ID");
		cmbSearch.add("User Name");
		...
		CheckBoxSelectionModel<TbLoginBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbLoginBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbLoginBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbLoginBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				pagingLoadConfig.set("type", "HR");
				interfaceAsync.getLoginPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbLoginBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbLoginBeanModel> result) {
		...
		pagingStore = new ListStore<TbLoginBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tblLoginId", "<center><b>User ID</b></center>", 150));
		columns.add(new ColumnConfig("tblUsername", "<center><b>User Name</b></center>", 150));
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowHRAdminUsersAddEdit window = WindowHRAdminUsersAddEdit.getInstance();
						window.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						MessageBox.confirm("Confirmation", "Are you sure you want to delete hr admin user \"" + model.get("tblLoginId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbLoginBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbLoginBeanModel>>() {
			public void handleEvent(GridEvent<TbLoginBeanModel> be) {
