...
import com.gwt.hris.client.service.admin.qualification.EducationInterfaceAsync;
...
public class WindowEducation extends WindowMain {
	private static WindowEducation instance = new WindowEducation();

	public static WindowEducation getInstance() {
		return instance;
	}

	public WindowEducation() {
		window = new Window();
		window.setId("WindowEducation");
		window.setSize(640, 380);
		window.setHeading("Qualification : Education");
	}

	EducationInterfaceAsync interfaceAsync = GWT.create(EducationInterface.class);
	...
	RpcProxy<PagingLoadResult<TbEducationBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbEducationBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbEducationBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteEducation(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbEducationBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbEducationBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbEducationBeanModel data = (TbEducationBeanModel) itr.next();
					ids[i] = data.getTbeId();
					i++;
				}
				interfaceAsync.deleteBulkEducation(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Education ID");
		cmbSearch.add("Course");
		cmbSearch.add("Institute");
		...
		CheckBoxSelectionModel<TbEducationBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbEducationBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbEducationBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbEducationBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getEducationPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbEducationBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbEducationBeanModel> result) {
		...
		pagingStore = new ListStore<TbEducationBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbeEducationId", "<center><b>Education ID</b></center>", 150));
		columns.add(new ColumnConfig("tbeInstitute", "<center><b>Institute</b></center>", 150));
		columns.add(new ColumnConfig("tbeCourse", "<center><b>Course</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbeId").toString());
						WindowEducationAddEdit windowEducationAddEdit = WindowEducationAddEdit.getInstance();
						windowEducationAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbeId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete education \"" + model.get("tbeEducationId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbEducationBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbEducationBeanModel>>() {
			public void handleEvent(GridEvent<TbEducationBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowEducationAddEdit windowEducationAddEdit = WindowEducationAddEdit.getInstance();
				windowEducationAddEdit.show("add", 0);
			}
		});
