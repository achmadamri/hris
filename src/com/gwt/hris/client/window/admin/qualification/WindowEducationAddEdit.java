...
public class WindowEducationAddEdit extends WindowMain {
	private static WindowEducationAddEdit instance = new WindowEducationAddEdit();

	public static WindowEducationAddEdit getInstance() {
		return instance;
	}

	public WindowEducationAddEdit() {
		window = new Window();
		window.setId("WindowEducationAddEdit");
	...
	TextField<String> txtInstitute = new TextField<String>();

	AsyncCallback<TbEducationBeanModel> getCallback = new AsyncCallback<TbEducationBeanModel>() {
		@Override
		public void onSuccess(TbEducationBeanModel result) {
			if (result.getOperationStatus()) {
				hfId.setValue(result.getTbeId().toString());

				lblCode.setValue(result.getTbeEducationId());
				txtInstitute.setValue(result.getTbeInstitute());
	...
	AsyncCallback<ReturnBean> submitCallback = new AsyncCallback<ReturnBean>() {
		...
		public void onSuccess(ReturnBean result) {
			...
			if (result.getOperationStatus()) {
				...
				TbEducationBeanModel beanModel = (TbEducationBeanModel) result.get("model");
				...
				WindowEducation.getInstance().bottomToolBar.refresh();
			} else {
				MessageBox.alert("Alert", "Operation Error.<br/><br/><br/>Error Message :<br/>" + result.getMessage(), null);
			}
		}
	...
	public void addComponents() {
		...
		txtInstitute.setId("txtTitle");
		txtInstitute.setFieldLabel("Institute *");
		txtInstitute.setAllowBlank(false);
		formPanel.add(txtInstitute, formData);
	...
	public void addButtons() {
		btnSave = new Button("Save", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				if (formPanel.isValid()) {
					...
					beanModel.setTbeInstitute(StringUtil.getInstance().getString(txtInstitute.getValue()));
					interfaceAsync.submitEducation(beanModel, submitCallback);
				} else {
		...
		btnCancel = new Button("Cancel", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				interfaceAsync.getEducation(id, getCallback);
