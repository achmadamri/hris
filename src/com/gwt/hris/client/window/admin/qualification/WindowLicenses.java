...
public class WindowLicenses extends WindowMain {
	private static WindowLicenses instance = new WindowLicenses();

	public static WindowLicenses getInstance() {
		return instance;
	}

	public WindowLicenses() {
		window = new Window();
		window.setId("WindowLicenses");
	...
	RpcProxy<PagingLoadResult<TbLicensesBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbLicensesBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbLicensesBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteLicenses(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbLicensesBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbLicensesBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbLicensesBeanModel data = (TbLicensesBeanModel) itr.next();
					ids[i] = data.getTblId();
					i++;
				}
				interfaceAsync.deleteBulkLicenses(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbLicensesBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbLicensesBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbLicensesBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbLicensesBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getLicensesPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbLicensesBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbLicensesBeanModel> result) {
		...
		pagingStore = new ListStore<TbLicensesBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tblLicensesId", "<center><b>Licenses ID</b></center>", 150));
		columns.add(new ColumnConfig("tblDescription", "<center><b>Description</b></center>", 150));
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowLicensesAddEdit windowLicensesAddEdit = WindowLicensesAddEdit.getInstance();
						windowLicensesAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						MessageBox.confirm("Confirmation", "Are you sure you want to delete licenses \"" + model.get("tblLicensesId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbLicensesBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbLicensesBeanModel>>() {
			public void handleEvent(GridEvent<TbLicensesBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowLicensesAddEdit windowLicensesAddEdit = WindowLicensesAddEdit.getInstance();
				windowLicensesAddEdit.show("add", 0);
			}
		});
