...
public class WindowProjectActivitiesGroup extends WindowMain {
	private static WindowProjectActivitiesGroup instance = new WindowProjectActivitiesGroup();

	public static WindowProjectActivitiesGroup getInstance() {
		return instance;
	}

	public WindowProjectActivitiesGroup() {
		window = new Window();
		window.setId("WindowProjectActivitiesGroup");
	...
	RpcProxy<PagingLoadResult<TbProjectActivitiesGroupBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbProjectActivitiesGroupBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbProjectActivitiesGroupBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteProjectActivitiesGroup(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbProjectActivitiesGroupBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbProjectActivitiesGroupBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbProjectActivitiesGroupBeanModel data = (TbProjectActivitiesGroupBeanModel) itr.next();
					ids[i] = data.getTbpagId();
					i++;
				}
				interfaceAsync.deleteBulkProjectActivitiesGroup(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Activity Group ID");
		...
		CheckBoxSelectionModel<TbProjectActivitiesGroupBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbProjectActivitiesGroupBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbProjectActivitiesGroupBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbProjectActivitiesGroupBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getProjectActivitiesGroupPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbProjectActivitiesGroupBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbProjectActivitiesGroupBeanModel> result) {
		...
		pagingStore = new ListStore<TbProjectActivitiesGroupBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbpagProjectActivitiesGroupId", "<center><b>Activity Group ID</b></center>", 150));
		columns.add(new ColumnConfig("tbpagName", "<center><b>Activity Group Name</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbpagId").toString());
						WindowProjectActivitiesGroupAddEdit windowProjectActivitiesGroupAddEdit = WindowProjectActivitiesGroupAddEdit.getInstance();
						windowProjectActivitiesGroupAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbpagId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete Activities Group \"" + model.get("tbpagProjectActivitiesGroupId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbProjectActivitiesGroupBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbProjectActivitiesGroupBeanModel>>() {
			public void handleEvent(GridEvent<TbProjectActivitiesGroupBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowProjectActivitiesGroupAddEdit windowProjectActivitiesGroupAddEdit = WindowProjectActivitiesGroupAddEdit.getInstance();
				windowProjectActivitiesGroupAddEdit.show("add", 0);
			}
		});
