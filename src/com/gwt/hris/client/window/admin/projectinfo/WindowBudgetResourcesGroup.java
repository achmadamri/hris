...
public class WindowBudgetResourcesGroup extends WindowMain {
	private static WindowBudgetResourcesGroup instance = new WindowBudgetResourcesGroup();

	public static WindowBudgetResourcesGroup getInstance() {
		return instance;
	}

	public WindowBudgetResourcesGroup() {
		window = new Window();
		window.setId("WindowBudgetResourcesGroup");
	...
	RpcProxy<PagingLoadResult<TbBudgetResourcesGroupBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbBudgetResourcesGroupBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbBudgetResourcesGroupBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteBudgetResourcesGroup(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbBudgetResourcesGroupBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbBudgetResourcesGroupBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbBudgetResourcesGroupBeanModel data = (TbBudgetResourcesGroupBeanModel) itr.next();
					ids[i] = data.getTbbrgId();
					i++;
				}
				interfaceAsync.deleteBulkBudgetResourcesGroup(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Budget Resources Group ID");
		cmbSearch.add("Name");
		...
		CheckBoxSelectionModel<TbBudgetResourcesGroupBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbBudgetResourcesGroupBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbBudgetResourcesGroupBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbBudgetResourcesGroupBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getBudgetResourcesGroupPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbBudgetResourcesGroupBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbBudgetResourcesGroupBeanModel> result) {
		...
		pagingStore = new ListStore<TbBudgetResourcesGroupBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbbrgBudgetResourcesGroupId", "<center><b>Budget Resources Group ID</b></center>", 150));
		columns.add(new ColumnConfig("tbbrgName", "<center><b>Name</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbbrgId").toString());
						WindowBudgetResourcesGroupAddEdit window = WindowBudgetResourcesGroupAddEdit.getInstance();
						window.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbbrgId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete Budget Resources Group \"" + model.get("tbbrgBudgetResourcesGroupId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbBudgetResourcesGroupBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbBudgetResourcesGroupBeanModel>>() {
			public void handleEvent(GridEvent<TbBudgetResourcesGroupBeanModel> be) {
