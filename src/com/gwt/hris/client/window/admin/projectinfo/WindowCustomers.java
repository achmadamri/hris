...
import com.gwt.hris.client.service.admin.projectinfo.CustomersInterfaceAsync;
...
public class WindowCustomers extends WindowMain {
	private static WindowCustomers instance = new WindowCustomers();

	public static WindowCustomers getInstance() {
		return instance;
	}

	public WindowCustomers() {
		window = new Window();
		window.setId("WindowCustomers");
		window.setSize(600, 380);
		window.setHeading("Project Info : Customers");
	}

	CustomersInterfaceAsync interfaceAsync = GWT.create(CustomersInterface.class);
	...
	RpcProxy<PagingLoadResult<TbCustomerBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbCustomerBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbCustomerBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteCustomer(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbCustomerBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbCustomerBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbCustomerBeanModel data = (TbCustomerBeanModel) itr.next();
					ids[i] = data.getTbcId();
					i++;
				}
				interfaceAsync.deleteBulkCustomer(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbCustomerBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbCustomerBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbCustomerBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbCustomerBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getCustomerPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbCustomerBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbCustomerBeanModel> result) {
		...
		pagingStore = new ListStore<TbCustomerBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbcCustomerId", "<center><b>Customer ID</b></center>", 150));
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowCustomersAddEdit windowCustomerAddEdit = WindowCustomersAddEdit.getInstance();
						windowCustomerAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbcId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete customer \"" + model.get("tbcCustomerId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbCustomerBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbCustomerBeanModel>>() {
			public void handleEvent(GridEvent<TbCustomerBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowCustomersAddEdit windowCustomerAddEdit = WindowCustomersAddEdit.getInstance();
				windowCustomerAddEdit.show("add", 0);
			}
		});
