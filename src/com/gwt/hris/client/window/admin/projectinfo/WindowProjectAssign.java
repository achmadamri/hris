...
public class WindowProjectAssign extends WindowMain {
	private static WindowProjectAssign instance = new WindowProjectAssign();

	public static WindowProjectAssign getInstance() {
		return instance;
	}

	public WindowProjectAssign() {
		window = new Window();
		window.setId("WindowProjectAssign");
		window.setSize(600, 380);
		window.setHeading("Project Info : Users Assign");
	}

	public void show(String strType, int intTbpId) {
	...
	ProjectInterfaceAsync projectInterfaceAsync = GWT.create(ProjectInterface.class);
	...
	AsyncCallback<ReturnBean> submitCallback = new AsyncCallback<ReturnBean>() {
		...
		public void onSuccess(ReturnBean result) {
			if (result.getOperationStatus()) {
				...
				windowProjectAddEdit.show("edit", intTbpId);
			} else {
	...
	Listener<MessageBoxEvent> assignListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				...
				beanModel.setTbeId(intTbeId);
				beanModel.setTbpId(intTbpId);
				projectInterfaceAsync.submitProjectEmployee(beanModel, submitCallback);
			}
		}
	};
