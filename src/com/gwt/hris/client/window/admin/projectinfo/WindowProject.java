...
public class WindowProject extends WindowMain {
	private static WindowProject instance = new WindowProject();

	public static WindowProject getInstance() {
		return instance;
	}

	public WindowProject() {
		window = new Window();
		window.setId("WindowProject");
		window.setSize(700, 380);
		window.setHeading("Project Info : Projects");
	}
	...
	RpcProxy<PagingLoadResult<ViewProjectCustomerBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<ViewProjectCustomerBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<ViewProjectCustomerBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteProject(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewProjectCustomerBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewProjectCustomerBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewProjectCustomerBeanModel data = (ViewProjectCustomerBeanModel) itr.next();
					ids[i] = data.getTbpId();
					i++;
				}
				interfaceAsync.deleteBulkProject(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<ViewProjectCustomerBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<ViewProjectCustomerBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<ViewProjectCustomerBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewProjectCustomerBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getProjectPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewProjectCustomerBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<ViewProjectCustomerBeanModel> result) {
		...
		pagingStore = new ListStore<ViewProjectCustomerBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbpProjectId", "<center><b>Project ID</b></center>", 100));
		columns.add(new ColumnConfig("tbpName", "<center><b>Project Name</b></center>", 150));
		columns.add(new ColumnConfig("tbcName", "<center><b>Customer Name</b></center>", 250));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbpId").toString());
						WindowProjectAddEdit windowProjectAddEdit = WindowProjectAddEdit.getInstance();
						windowProjectAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbpId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete project \"" + model.get("tbpProjectId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<ViewProjectCustomerBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<ViewProjectCustomerBeanModel>>() {
			public void handleEvent(GridEvent<ViewProjectCustomerBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				WindowProjectAddEdit window = WindowProjectAddEdit.getInstance();
				window.show("add", 0);
			}
		});
