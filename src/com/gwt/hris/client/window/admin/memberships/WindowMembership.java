...
public class WindowMembership extends WindowMain {
	private static WindowMembership instance = new WindowMembership();

	public static WindowMembership getInstance() {
		return instance;
	}

	public WindowMembership() {
		window = new Window();
		window.setId("WindowMembership");
		window.setSize(620, 380);
	...
	RpcProxy<PagingLoadResult<ViewMembershipTypesBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<ViewMembershipTypesBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<ViewMembershipTypesBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteMembership(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewMembershipTypesBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewMembershipTypesBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewMembershipTypesBeanModel data = (ViewMembershipTypesBeanModel) itr.next();
					ids[i] = data.getTbmId();
					i++;
				}
				interfaceAsync.deleteBulkMembership(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<ViewMembershipTypesBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<ViewMembershipTypesBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<ViewMembershipTypesBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewMembershipTypesBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getMembershipPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewMembershipTypesBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<ViewMembershipTypesBeanModel> result) {
		...
		pagingStore = new ListStore<ViewMembershipTypesBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbmMembershipId", "<center><b>Membership ID</b></center>", 150));
		columns.add(new ColumnConfig("tbmName", "<center><b>Membership Name</b></center>", 150));
		columns.add(new ColumnConfig("tbmtName", "<center><b>Membership Type</b></center>", 150));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbmId").toString());
						WindowMembershipAddEdit windowMembershipAddEdit = WindowMembershipAddEdit.getInstance();
						windowMembershipAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbmId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete membership \"" + model.get("tbmMembershipId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<ViewMembershipTypesBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<ViewMembershipTypesBeanModel>>() {
			public void handleEvent(GridEvent<ViewMembershipTypesBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowMembershipAddEdit windowMembershipAddEdit = WindowMembershipAddEdit.getInstance();
				windowMembershipAddEdit.show("add", 0);
			}
		});
