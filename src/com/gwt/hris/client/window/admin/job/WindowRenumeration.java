...
public class WindowRenumeration extends WindowMain {
	private static WindowRenumeration instance = new WindowRenumeration();

	public static WindowRenumeration getInstance() {
		return instance;
	}

	public WindowRenumeration() {
		window = new Window();
		window.setId("WindowRenumeration");
		window.setSize(710, 380);
	...
	RpcProxy<PagingLoadResult<ViewRenumerationBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<ViewRenumerationBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<ViewRenumerationBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteRenumeration(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewRenumerationBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewRenumerationBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewRenumerationBeanModel data = (ViewRenumerationBeanModel) itr.next();
					ids[i] = data.getTbrId();
					i++;
				}
				interfaceAsync.deleteBulkRenumeration(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Renumeration ID");
		cmbSearch.add("Renumeration Name");
		...
		CheckBoxSelectionModel<ViewRenumerationBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<ViewRenumerationBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<ViewRenumerationBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewRenumerationBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getRenumerationPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewRenumerationBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<ViewRenumerationBeanModel> result) {
		...
		pagingStore = new ListStore<ViewRenumerationBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbrRenumerationId", "<center><b>Renumeration ID</b></center>", 150));
		columns.add(new ColumnConfig("tbrName", "<center><b>Renumeration Name</b></center>", 150));
		columns.add(new ColumnConfig("tbcCurrencyId", "<center><b>Currency</b></center>", 75));

		ColumnConfig columnConfig = new ColumnConfig("tbrNominal", "<center><b>Nominal</b></center>", 150);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				double data1 = model.get("tbrNominal");
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbrId").toString());
						WindowRenumerationAddEdit windowRenumerationAddEdit = WindowRenumerationAddEdit.getInstance();
						windowRenumerationAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbrId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete Renumeration \"" + model.get("tbrRenumerationId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<ViewRenumerationBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<ViewRenumerationBeanModel>>() {
			public void handleEvent(GridEvent<ViewRenumerationBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowRenumerationAddEdit windowRenumerationAddEdit = WindowRenumerationAddEdit.getInstance();
				windowRenumerationAddEdit.show("add", 0);
			}
		});
