...
import com.gwt.hris.client.service.admin.job.EmploymentStatusInterfaceAsync;
...
public class WindowEmploymentStatus extends WindowMain {
	private static WindowEmploymentStatus instance = new WindowEmploymentStatus();

	public static WindowEmploymentStatus getInstance() {
		return instance;
	}

	public WindowEmploymentStatus() {
		window = new Window();
		window.setId("WindowEmploymentStatus");
		window.setSize(600, 380);
		window.setHeading("Job : Employment Status");
	}

	EmploymentStatusInterfaceAsync employmentStatusInterfaceAsync = GWT.create(EmploymentStatusInterface.class);
	...
	RpcProxy<PagingLoadResult<TbEmploymentStatusBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbEmploymentStatusBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbEmploymentStatusBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				employmentStatusInterfaceAsync.deleteEmploymentStatus(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbEmploymentStatusBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbEmploymentStatusBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbEmploymentStatusBeanModel data = (TbEmploymentStatusBeanModel) itr.next();
					ids[i] = data.getTbesId();
					i++;
				}
				employmentStatusInterfaceAsync.deleteBulkEmploymentStatus(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Emp. Stat. ID");
		cmbSearch.add("Emp. Stat. Name");
		...
		CheckBoxSelectionModel<TbEmploymentStatusBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbEmploymentStatusBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbEmploymentStatusBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbEmploymentStatusBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				employmentStatusInterfaceAsync.getEmploymentStatusPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbEmploymentStatusBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbEmploymentStatusBeanModel> result) {
		...
		pagingStore = new ListStore<TbEmploymentStatusBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbesEmploymentStatusId", "<center><b>Employment Status ID</b></center>", 180));
		columns.add(new ColumnConfig("tbesName", "<center><b>Employment Status Name</b></center>", 200));
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowEmploymentStatusAddEdit windowEmploymentStatusAddEdit = WindowEmploymentStatusAddEdit.getInstance();
						windowEmploymentStatusAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						MessageBox.confirm("Confirmation", "Are you sure you want to delete employment status \"" + model.get("tbesName") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbEmploymentStatusBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbEmploymentStatusBeanModel>>() {
			public void handleEvent(GridEvent<TbEmploymentStatusBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowEmploymentStatusAddEdit windowEmploymentStatusAddEdit = WindowEmploymentStatusAddEdit.getInstance();
				windowEmploymentStatusAddEdit.show("add", 0);
			}
		});
