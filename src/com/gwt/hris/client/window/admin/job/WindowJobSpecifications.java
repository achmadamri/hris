...
public class WindowJobSpecifications extends WindowMain {
	private static WindowJobSpecifications instance = new WindowJobSpecifications();

	public static WindowJobSpecifications getInstance() {
		return instance;
	}

	public WindowJobSpecifications() {
		window = new Window();
		window.setId("WindowJobSpecifications");
		window.setSize(600, 380);
		window.setHeading("Job : Job Specifications");
	}

	JobSpecificationsInterfaceAsync paygradeInterfaceAsync = GWT.create(JobSpecificationsInterface.class);
	...
	RpcProxy<PagingLoadResult<TbJobSpecificationsBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbJobSpecificationsBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbJobSpecificationsBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				paygradeInterfaceAsync.deleteJobSpecifications(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbJobSpecificationsBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbJobSpecificationsBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbJobSpecificationsBeanModel data = (TbJobSpecificationsBeanModel) itr.next();
					ids[i] = data.getTbjsId();
					i++;
				}
				paygradeInterfaceAsync.deleteBulkJobSpecifications(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbJobSpecificationsBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbJobSpecificationsBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbJobSpecificationsBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbJobSpecificationsBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				paygradeInterfaceAsync.getJobSpecifications(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbJobSpecificationsBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbJobSpecificationsBeanModel> result) {
		...
		pagingStore = new ListStore<TbJobSpecificationsBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbjsName", "<center><b>Job Spec Name</b></center>", 100));
		columns.add(new ColumnConfig("tbjsDescription", "<center><b>Description</b></center>", 200));
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbjsId").toString());
						WindowJobSpecificationsAddEdit windowJobSpecificationsAddEdit = WindowJobSpecificationsAddEdit.getInstance();
						windowJobSpecificationsAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbjsId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete job specifications \"" + model.get("tbjsName") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbJobSpecificationsBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbJobSpecificationsBeanModel>>() {
			public void handleEvent(GridEvent<TbJobSpecificationsBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowJobSpecificationsAddEdit windowJobSpecificationsAddEdit = WindowJobSpecificationsAddEdit.getInstance();
				windowJobSpecificationsAddEdit.show("add", 0);
			}
		});
