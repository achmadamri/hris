...
public class WindowPaygrade extends WindowMain {
	private static WindowPaygrade instance = new WindowPaygrade();

	public static WindowPaygrade getInstance() {
		return instance;
	}

	public WindowPaygrade() {
		window = new Window();
		window.setId("WindowPaygrade");
		window.setSize(600, 380);
		window.setHeading("Job : Salary Grades");
	}
	...
	RpcProxy<PagingLoadResult<TbPaygradeBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbPaygradeBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbPaygradeBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				paygradeInterfaceAsync.deletePaygrade(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbPaygradeBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbPaygradeBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbPaygradeBeanModel data = (TbPaygradeBeanModel) itr.next();
					ids[i] = data.getTbpId();
					i++;
				}
				paygradeInterfaceAsync.deleteBulkPaygrade(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		cmbSearch.add("Salary ID");
		cmbSearch.add("Salary Name");
		...
		CheckBoxSelectionModel<TbPaygradeBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbPaygradeBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbPaygradeBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbPaygradeBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				paygradeInterfaceAsync.getPaygradePaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbPaygradeBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbPaygradeBeanModel> result) {
		...
		pagingStore = new ListStore<TbPaygradeBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbpPaygradeId", "<center><b>Salary ID</b></center>", 100));
		columns.add(new ColumnConfig("tbpName", "<center><b>Salary Name</b></center>", 200));
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						WindowPaygradeAddEdit windowPaygradeAddEdit = WindowPaygradeAddEdit.getInstance();
						windowPaygradeAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						MessageBox.confirm("Confirmation", "Are you sure you want to delete salary \"" + model.get("tbpName") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbPaygradeBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbPaygradeBeanModel>>() {
			public void handleEvent(GridEvent<TbPaygradeBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowPaygradeAddEdit windowPaygradeAddEdit = WindowPaygradeAddEdit.getInstance();
				windowPaygradeAddEdit.show("add", 0);
			}
		});
