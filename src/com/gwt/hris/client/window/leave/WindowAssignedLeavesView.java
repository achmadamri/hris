...
public class WindowAssignedLeavesView extends WindowMain {
	private static WindowAssignedLeavesView instance = new WindowAssignedLeavesView();

	public static WindowAssignedLeavesView getInstance() {
		return instance;
	}

	public WindowAssignedLeavesView() {
		window = new Window();
		window.setId("WindowAssignedLeavesView");
				...
				if (((Integer) result.get("leaveAvailable")) >= 0) {
					if (result.get("leaveTaken") != null) {
						if (((Integer) result.get("leaveTaken")) >= 0) {
							...
							dateStart.clear();
							dateEnd.clear();
							assignedLeavesInterfaceAsync.getLeaveCount(null, null, 1, 0, leaveCountCallback);
						}
					} else {
						lblLeaveEntitled.setValue(result.get("leaveEntitled"));
						lblLeaveAvailable.setValue(result.get("leaveAvailable"));
	...
	AsyncCallback<TbAssignedLeavesBeanModel> getAssignedLeavesCallback = new AsyncCallback<TbAssignedLeavesBeanModel>() {
		...
		public void onSuccess(TbAssignedLeavesBeanModel result) {
			if (result.getOperationStatus()) {
				...
				assignedLeavesInterfaceAsync.getLeaveCount(result.getTbaleStartDate(), result.getTbaleEndDate(), 0, 0, leaveCountCallback);
	...
	public void addComponents() {
		...
		dateStart.addListener(Events.Valid, new Listener<BaseEvent>() {
			...
			public void handleEvent(BaseEvent be) {
				if (isLocked == 0) {
					assignedLeavesInterfaceAsync.getLeaveCount(dateStart.getValue(), dateEnd.getValue(), 1, 0, leaveCountCallback);
				}
			}
		});
