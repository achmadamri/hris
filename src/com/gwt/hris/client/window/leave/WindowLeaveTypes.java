...
public class WindowLeaveTypes extends WindowMain {
	private static WindowLeaveTypes instance = new WindowLeaveTypes();

	public static WindowLeaveTypes getInstance() {
		return instance;
	}

	public WindowLeaveTypes() {
		window = new Window();
		window.setId("WindowLeaveTypes");
	...
	RpcProxy<PagingLoadResult<TbLeaveTypesBeanModel>> proxy;
	PagingLoader<PagingLoadResult<ModelData>> pagingLoader;
	ListStore<TbLeaveTypesBeanModel> pagingStore;
	PagingToolBar bottomToolBar = new PagingToolBar(20);
	Grid<TbLeaveTypesBeanModel> grid;
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteLeaveTypes(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<TbLeaveTypesBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<TbLeaveTypesBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					TbLeaveTypesBeanModel data = (TbLeaveTypesBeanModel) itr.next();
					ids[i] = data.getTbltId();
					i++;
				}
				interfaceAsync.deleteBulkLeaveTypes(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<TbLeaveTypesBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<TbLeaveTypesBeanModel>();

		proxy = new RpcProxy<PagingLoadResult<TbLeaveTypesBeanModel>>() {
			@Override
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<TbLeaveTypesBeanModel>> callback) {
				PagingLoadConfig pagingLoadConfig = (PagingLoadConfig) loadConfig;
				pagingLoadConfig.set("limit", bottomToolBar.getPageSize());
				interfaceAsync.getLeaveTypesPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<TbLeaveTypesBeanModel>>() {
					@Override
					public void onSuccess(PagingLoadResult<TbLeaveTypesBeanModel> result) {
		...
		pagingStore = new ListStore<TbLeaveTypesBeanModel>(pagingLoader);

		bottomToolBar.bind(pagingLoader);

		List<ColumnConfig> columns = new ArrayList<ColumnConfig>();
		columns.add(checkBoxSelectionModel.getColumn());
		columns.add(new ColumnConfig("tbltLeaveTypesId", "<center><b>Leave Types ID</b></center>", 100));
		columns.add(new ColumnConfig("tbltName", "<center><b>Leave Types</b></center>", 200));

		ColumnConfig columnConfig = new ColumnConfig("tbltReduction", "<center><b>Reduction</b></center>", 110);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				Integer data = model.get("tbltReduction");
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						int id = Integer.parseInt(model.get("tbltId").toString());
						WindowLeaveTypesAddEdit windowLeaveTypesAddEdit = WindowLeaveTypesAddEdit.getInstance();
						windowLeaveTypesAddEdit.show("edit", id);
					}
				});
				...
				Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						deleteID = Integer.parseInt(model.get("tbltId").toString());

						MessageBox.confirm("Confirmation", "Are you sure you want to delete leave types \"" + model.get("tbltLeaveTypesId") + "\" ?", deleteListener);
					}
				});
		...
		grid = new Grid<TbLeaveTypesBeanModel>(pagingStore, cm);
		grid.addListener(Events.Attach, new Listener<GridEvent<TbLeaveTypesBeanModel>>() {
			public void handleEvent(GridEvent<TbLeaveTypesBeanModel> be) {
	...
	public void addButtons() {
		btnAdd = new Button("Add", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				WindowLeaveTypesAddEdit windowLeaveTypesAddEdit = WindowLeaveTypesAddEdit.getInstance();
				windowLeaveTypesAddEdit.show("add", 0);
			}
		});
