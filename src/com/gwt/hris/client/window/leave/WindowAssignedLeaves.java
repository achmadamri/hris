...
public class WindowAssignedLeaves extends WindowMain {
	private static WindowAssignedLeaves instance = new WindowAssignedLeaves();

	public static WindowAssignedLeaves getInstance() {
		return instance;
	}

	public WindowAssignedLeaves() {
		window = new Window();
		window.setId("WindowAssignedLeaves");
		window.setSize(700, 380);
		window.setHeading("Leave : Assigned Leaves");
	}

	AssignedLeavesInterfaceAsync interfaceAsync = GWT.create(AssignedLeavesInterface.class);
	...
	Listener<MessageBoxEvent> deleteListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				interfaceAsync.deleteAssignedLeaves(deleteID, callback);
			}
		}
	};
	...
	Listener<MessageBoxEvent> deleteBulkListener = new Listener<MessageBoxEvent>() {
		public void handleEvent(MessageBoxEvent ce) {
			...
			if ("Yes".equals(strBtnText)) {
				List<ViewEmployeeLeavesBeanModel> lst = grid.getSelectionModel().getSelectedItems();

				Integer ids[] = new Integer[lst.size()];
				Iterator<ViewEmployeeLeavesBeanModel> itr = lst.iterator();
				int i = 0;
				while (itr.hasNext()) {
					ViewEmployeeLeavesBeanModel data = (ViewEmployeeLeavesBeanModel) itr.next();
					ids[i] = data.getTbaleId();
					i++;
				}
				interfaceAsync.deleteBulkAssignedLeaves(ids, callback);
			}
		}
	};
	...
	public void addComponents() {
		...
		CheckBoxSelectionModel<ViewEmployeeLeavesBeanModel> checkBoxSelectionModel = new CheckBoxSelectionModel<ViewEmployeeLeavesBeanModel>();
		...
		proxy = new RpcProxy<PagingLoadResult<ViewEmployeeLeavesBeanModel>>() {
			...
			protected void load(Object loadConfig, final AsyncCallback<PagingLoadResult<ViewEmployeeLeavesBeanModel>> callback) {
				...
				interfaceAsync.getTbAssignedLeavesPaging(pagingLoadConfig, cmbSearch.getSimpleValue(), txtSearch.getValue(), new AsyncCallback<PagingLoadResult<ViewEmployeeLeavesBeanModel>>() {
					@Override
				...
				if (data == 0) {
					...
					Button btnEdit = new Button("", new SelectionListener<ButtonEvent>() {
						...
						public void componentSelected(ButtonEvent ce) {
							...
							WindowAssignedLeavesAddEdit windowAssignedLeavesAdd = WindowAssignedLeavesAddEdit.getInstance();
							windowAssignedLeavesAdd.show("edit", id);
						}
					});
					...
					Button btnDelete = new Button("", new SelectionListener<ButtonEvent>() {
						@Override
						public void componentSelected(ButtonEvent ce) {
							deleteID = Integer.parseInt(model.get("tbaleId").toString());

							DateTimeFormat fmt = DateTimeFormat.getFormat("yyyy-MM-dd");
							Date dateStart = model.get("tbaleStartDate");
							Date dateEnd = model.get("tbaleEndDate");

							MessageBox.confirm("Confirmation", "Are you sure you want to delete leave from " + fmt.format(dateStart) + " to " + fmt.format(dateEnd) + " ?", deleteListener);
						}
					});
					...
					Button btnView = new Button("", new SelectionListener<ButtonEvent>() {
						...
						public void componentSelected(ButtonEvent ce) {
							int id = Integer.parseInt(model.get("tbaleId").toString());
							WindowAssignedLeavesView windowAssignedLeavesView = WindowAssignedLeavesView.getInstance();
							windowAssignedLeavesView.show("view", id);
						}
					});
	...
	public void addButtons() {
		btnApply = new Button("Apply", new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				WindowAssignedLeavesAddEdit windowAssignedLeavesAdd = WindowAssignedLeavesAddEdit.getInstance();
				windowAssignedLeavesAdd.show("add", 0);
			}
		});
		window.addButton(btnApply);
