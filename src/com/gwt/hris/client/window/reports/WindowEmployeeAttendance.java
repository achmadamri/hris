...
public class WindowEmployeeAttendance extends WindowMain {
	private static WindowEmployeeAttendance instance = new WindowEmployeeAttendance();

	public static WindowEmployeeAttendance getInstance() {
		return instance;
	}

	public WindowEmployeeAttendance() {
		window = new Window();
		window.setId("WindowEmployeeAttendance");
		window.setSize(940, 380);
		window.setHeading("Reports : Employee Attendance");
	}
	...
	public void addComponents() {
		...
		ColumnConfig columnConfig = new ColumnConfig("rtbaInLongitude", "<center><b>In Long-Lat</b></center>", 90);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				String longitude = model.get("rtbaInLongitude");
				String latitude= model.get("rtbaInLatitude");
		...
		columnConfig = new ColumnConfig("rtbaOutLongitude", "<center><b>Out Long-Lat</b></center>", 90);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				String longitude = model.get("rtbaOutLongitude");
				String latitude= model.get("rtbaOutLatitude");
		...
		columnConfig = new ColumnConfig("rtbaInTimeDiff", "<center><b>In Late</b></center>", 70);
		columnConfig.setRenderer(new GridCellRenderer<ModelData>() {
			@Override
			public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
		...
		ccActions.setRenderer(new GridCellRenderer<ModelData>() {
			...
			public Object render(final ModelData model, String property, ColumnData config, int rowIndex, int colIndex, ListStore<ModelData> store, Grid<ModelData> grid) {
				...
				if (model.get("rtbaInPhoto") != null) {
					Button btnProofIn = new Button("", new SelectionListener<ButtonEvent>() {
						@Override
						public void componentSelected(ButtonEvent ce) {
							String folderPath = GWT.getHostPageBaseURL() + "upload/photoattendance/";
							String fileName = model.get("rtbaInPhoto");
					...
					btnProofIn.setToolTip("proof in");
					btnProofIn.setIcon(Resources.ICONS.menu_show());
					layoutContainer.add(btnProofIn, hBoxLayoutData);
				}

				if (model.get("rtbaOutPhoto") != null) {
					Button btnProofOut = new Button("", new SelectionListener<ButtonEvent>() {
						@Override
						public void componentSelected(ButtonEvent ce) {
							String folderPath = GWT.getHostPageBaseURL() + "upload/photoattendance/";
							String fileName = model.get("rtbaOutPhoto");
							Window wndImage = new Window();
							wndImage.setSize(500, 500);
							wndImage.setLayout(new FitLayout());
							wndImage.setClosable(true);
					...
					btnProofOut.setToolTip("proof out");
					btnProofOut.setIcon(Resources.ICONS.menu_show());
					layoutContainer.add(btnProofOut, hBoxLayoutData);
				}
