// ______________________________________________________
// Generated by sql2java - http://sql2java.sourceforge.net/
// jdbc driver used at code generation time: com.mysql.jdbc.Driver
//
// Please help us improve this tool by reporting:
// - problems and suggestions to
//   http://sourceforge.net/tracker/?group_id=54687
// - feedbacks and ideas on
//   http://sourceforge.net/forum/forum.php?forum_id=182208
// ______________________________________________________

package com.gwt.hris.client.service.bean;

public class TbCountryBeanService
{
	private static final long serialVersionUID = -3572683760511939709L;
	
    private String tbcName;

    private Integer tbcId;

	
	private TbCountryBeanService beanServices[];
	
	public TbCountryBeanService[] getServices() {
		return beanServices;
	}

	public void setServices(TbCountryBeanService[] _beanServices) {
		this.beanServices = _beanServices;
	}

    public String getTbcName()
    {
		return tbcName;
    }

    public void setTbcName(String newVal)
    {
		this.tbcName = newVal;
    }


    public Integer getTbcId()
    {
		return tbcId;
    }

    public void setTbcId(Integer newVal)
    {
		this.tbcId = newVal;
    }

    public void setTbcId(int newVal)
    {
        setTbcId(new Integer(newVal));
    }

}
