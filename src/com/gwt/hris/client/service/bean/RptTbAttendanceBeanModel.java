// ______________________________________________________
// Generated by sql2java - http://sql2java.sourceforge.net/
// jdbc driver used at code generation time: com.mysql.jdbc.Driver
//
// Please help us improve this tool by reporting:
// - problems and suggestions to
//   http://sourceforge.net/tracker/?group_id=54687
// - feedbacks and ideas on
//   http://sourceforge.net/forum/forum.php?forum_id=182208
// ______________________________________________________

package com.gwt.hris.client.service.bean;

import java.io.Serializable;

public class RptTbAttendanceBeanModel extends ReturnBean implements Serializable
{
	private static final long serialVersionUID = 8408901540235116631L;
	
	private RptTbAttendanceBeanModel beanModels[];
	
	public RptTbAttendanceBeanModel[] getModels() {
		return beanModels;
	}

	public void setModels(RptTbAttendanceBeanModel[] _beanModels) {
		this.beanModels = _beanModels;
	}

    public String getRtbaOutLongitude()
    {
		return (String) get("rtbaOutLongitude");
    }

    public void setRtbaOutLongitude(String newVal)
    {
		set("rtbaOutLongitude", newVal);
    }


    public String getRtbaOutPhoto()
    {
		return (String) get("rtbaOutPhoto");
    }

    public void setRtbaOutPhoto(String newVal)
    {
		set("rtbaOutPhoto", newVal);
    }


    public String getRtbaInPhoto()
    {
		return (String) get("rtbaInPhoto");
    }

    public void setRtbaInPhoto(String newVal)
    {
		set("rtbaInPhoto", newVal);
    }


    public String getRtbaOutLatitude()
    {
		return (String) get("rtbaOutLatitude");
    }

    public void setRtbaOutLatitude(String newVal)
    {
		set("rtbaOutLatitude", newVal);
    }


    public String getRtbaInLatitude()
    {
		return (String) get("rtbaInLatitude");
    }

    public void setRtbaInLatitude(String newVal)
    {
		set("rtbaInLatitude", newVal);
    }


    public String getRtbaInLongitude()
    {
		return (String) get("rtbaInLongitude");
    }

    public void setRtbaInLongitude(String newVal)
    {
		set("rtbaInLongitude", newVal);
    }


    public Integer getTbsId()
    {
		return (Integer) get("tbsId");
    }

    public void setTbsId(Integer newVal)
    {
		set("tbsId", newVal);
    }

    public void setTbsId(int newVal)
    {
        setTbsId(new Integer(newVal));
    }

    public Integer getRtbaOutTimeDiff()
    {
		return (Integer) get("rtbaOutTimeDiff");
    }

    public void setRtbaOutTimeDiff(Integer newVal)
    {
		set("rtbaOutTimeDiff", newVal);
    }

    public void setRtbaOutTimeDiff(int newVal)
    {
        setRtbaOutTimeDiff(new Integer(newVal));
    }

    public Integer getRtbaInTimeDiff()
    {
		return (Integer) get("rtbaInTimeDiff");
    }

    public void setRtbaInTimeDiff(Integer newVal)
    {
		set("rtbaInTimeDiff", newVal);
    }

    public void setRtbaInTimeDiff(int newVal)
    {
        setRtbaInTimeDiff(new Integer(newVal));
    }

    public String getRtbaOutNote()
    {
		return (String) get("rtbaOutNote");
    }

    public void setRtbaOutNote(String newVal)
    {
		set("rtbaOutNote", newVal);
    }


    public String getRtbaInNote()
    {
		return (String) get("rtbaInNote");
    }

    public void setRtbaInNote(String newVal)
    {
		set("rtbaInNote", newVal);
    }


    public String getRtbaOutTime()
    {
		return (String) get("rtbaOutTime");
    }

    public void setRtbaOutTime(String newVal)
    {
		set("rtbaOutTime", newVal);
    }


    public String getRtbaInTime()
    {
		return (String) get("rtbaInTime");
    }

    public void setRtbaInTime(String newVal)
    {
		set("rtbaInTime", newVal);
    }


    public String getRtbaDate()
    {
		return (String) get("rtbaDate");
    }

    public void setRtbaDate(String newVal)
    {
		set("rtbaDate", newVal);
    }


    public Integer getTbeId()
    {
		return (Integer) get("tbeId");
    }

    public void setTbeId(Integer newVal)
    {
		set("tbeId", newVal);
    }

    public void setTbeId(int newVal)
    {
        setTbeId(new Integer(newVal));
    }

    public Integer getRtbaId()
    {
		return (Integer) get("rtbaId");
    }

    public void setRtbaId(Integer newVal)
    {
		set("rtbaId", newVal);
    }

    public void setRtbaId(int newVal)
    {
        setRtbaId(new Integer(newVal));
    }

}
